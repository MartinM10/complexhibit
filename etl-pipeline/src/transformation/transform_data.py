"""Transform Data - RDF Triple Generation

Transforms normalized relational data from pathwise.db into RDF triples
following the OntoExhibit ontology.

This module contains the core transformation logic for all entity types:
- Personas (persons and groups)
...
from rdflib import URIRef, Literal, RDF, XSD, RDFS
from urllib.parse import urlparse
import pandas as pd

# Import from our modular utilities (using absolute imports for compatibility)
from transformation.rdf_builder import (
    rdf_graph, ontoexhibit, URI_ONTOLOGIA, URI_GEONAMES,
    CLASSES, serialize_graph, get_graph_stats
)
from transformation.utils import (
    hash_sha256, validar_fecha, procesar_coordenadas, 
    is_valid_url, is_valid_name, dataframe_from_cursor,
    is_approximate_date, get_date_stats, BANNED_LIST
)

# Optional translator - import only if needed
try:
...
# Re-export class mappings for backward compatibility
classes = CLASSES


# Helper function for backward compatibility with existing code
def dataframe_from_sql_query(results: Cursor) -> pd.DataFrame:
    """Convert SQL query results to DataFrame. Uses global cursor for column names.
    
    Replaces NaN values with None to avoid float type issues later.
    """
    df = pd.DataFrame(results.fetchall())
    if len(df) > 0:
        field_names = [i[0] for i in cursor.description]
        df.columns = field_names
        # Convert all object columns to string, replacing NaN with empty string
...
def obtener_primer_texto(cadena: str) -> str | None:
    """Get first non-empty text segment from semicolon-separated string."""
    if not cadena:
        return None
...
def sanitize_value(value) -> str | None:
    """
    Sanitize a value from pandas DataFrame to handle NaN, None, and 'None' strings.
    
    Returns None for invalid values, or the string value otherwise.
    """
    import math
    ...
    # Handle pandas NaN and numpy nan (which are floats)
    if isinstance(value, float):
        if math.isnan(value):
            return None
        # For other floats, convert to string
        return str(value)
...
def obtain_data_from_personas():
    ...
    for row in df.to_dict(orient="records"):
        ...
        name = sanitize_value(row['nombre'])
        tipo = sanitize_value(row['tipo'])
        genero = sanitize_value(row['género'])
        lugar_origen = sanitize_value(row['lugar de origen'])
        fecha_nacimiento = row['fecha de nacimiento']  # validated by validar_fecha
        fecha_defuncion = row['fecha de defunción']  # validated by validar_fecha
        direccion_postal = sanitize_value(row['dirección postal'])
        coordenadas = sanitize_value(row['coordenadas'])
        ...
        if name and name != 'None' and 'sin determinar' not in str(name).lower():
            ...
            if fecha_nacimiento:
                ...
                if (fecha_nacimiento.month == 1 and fecha_nacimiento.day == 1) \
                    ...
                    rdf_graph.add(
                        (uri_fecha_nacimiento, RDFS.label, Literal(f"{fecha_nacimiento}", datatype=XSD['gYear'])))
                else:
            ...
            if fecha_defuncion:
                ...
                if ((fecha_defuncion.month == 1 and fecha_defuncion.day == 1) or
                    ...
                    rdf_graph.add((uri_fecha, RDFS.label, Literal(f"{fecha_defuncion}", datatype=XSD['gYear'])))
...
def obtain_data_from_institucion():
    ...
    for row in df.to_dict(orient="records"):
        expofinder_id = row['id']
        name = sanitize_value(row['nombre'])
        nombre_alternativo = sanitize_value(row['nombre alternativo'])
        tipo = sanitize_value(row['tipo'])
        tipo_persona = sanitize_value(row['tipo_persona'])
        titularidad = sanitize_value(row['titularidad'])
        lugar_sede = sanitize_value(row['lugar de la sede'])
        direccion_sede = sanitize_value(row['dirección postal'])
        maximo_responsable = sanitize_value(row['máximo responsable'])
        coordenadas = sanitize_value(row['coordenadas'])

        uri_html = sanitize_value(row['URI HTML'])
        uri_rss = sanitize_value(row['URI RSS'])
        email = sanitize_value(row['correo electrónico'])
        fax = sanitize_value(row['fax'])
        telefono = sanitize_value(row['teléfono'])
        pagina_web = sanitize_value(row['página web'])
...
def obtain_data_from_empresa():
    ...
    for row in df.to_dict(orient="records"):
        expofinder_id = row['id']
        name = sanitize_value(row['nombre'])
        categoria = sanitize_value(row['categoria'])
        dimension = sanitize_value(row['dimensión'])
        lugar_sede = sanitize_value(row['lugar de la sede'])
...
def obtain_data_from_obras():
    ...
    for row in df.to_dict(orient="records"):
        expofinder_id = row['id']
        nombre_obra = sanitize_value(row['nombre_obra'])
        nombre_alternativo = sanitize_value(row['título alternativo'])
        fecha_comienzo = row['fecha de comienzo']  # validated later
        fecha_terminacion = row['fecha de terminación']  # validated later
        lugar_creacion_obra = sanitize_value(row['lugar de creación de la obra'])
        nombre_persona = sanitize_value(row['nombre_persona'])
        tipo_obra = sanitize_value(row['tipo_obra'])
        tipo_persona = sanitize_value(row['tipo_persona'])
...
def obtain_data_from_exposicion():
    ...
    for row in df.to_dict(orient="records"):
        expofinder_id = row['id']
        name = sanitize_value(row['nombre'])
        acceso = sanitize_value(row['acceso'])
        coordenadas = sanitize_value(row['coordenadas'])
        fecha_apertura = row['fecha de apertura']  # validated later
        fecha_cierre = row['fecha de cierre']  # validated later
        lugar_celebracion = sanitize_value(row['lugar donde se celebra'])
        sede = sanitize_value(row['sede'])
        tipo = sanitize_value(row['tipo'])
    ...
    for row in df.to_dict(orient="records"):
        nombre_exposicion = sanitize_value(row['nombre_exposicion'])
        nombre_catalogo = sanitize_value(row['nombre_catalogo'])
        id_exposicion = row['id_exposicion']
        id_catalogo = row['id_catalogo']
        fecha_publicacion = row['fecha_publicacion']
        lugar_publicacion = sanitize_value(row['lugar_publicacion'])
        tipo_catalogo = sanitize_value(row['tipo_catalogo'])
        id_editorial = row['id_editorial']
        nombre_editorial = sanitize_value(row['nombre_editorial'])
...
# Print date validation statistics
stats = get_date_stats()
if stats['total'] > 0:
    print(f"Date validation: {stats['success']}/{stats['total']} valid ({stats['success_pct']}%)")
    print(f"Invalid dates: {stats['fail']} ({stats['fail_pct']}%)")

# Print graph statistics
graph_stats = get_graph_stats()
print(f"Generated {graph_stats['num_triples']} triples for {graph_stats['num_subjects']} subjects")

########################################################################################################################
# SAVE OUTPUT FILE
########################################################################################################################

serialize_graph('result.nt')
